...
class FrameMeta : public Meta {
private:
    /* data */
public:
    FrameMeta(cv::Mat frame, int frame_index = -1, int channel_index = -1, int original_width = 0, int original_height = 0, int fps = 0);
    ~FrameMeta();

    // define copy constructor since we need deep copy operation.
    FrameMeta(const FrameMeta &meta);
