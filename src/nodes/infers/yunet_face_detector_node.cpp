...
YunetFaceDetectorNode::YunetFaceDetectorNode(std::string node_name,
                                             std::string model_path,
                                             float score_threshold,
                                             float nms_threshold,
                                             int top_k) :
    PrimaryInferNode(node_name, model_path),
...
YunetFaceDetectorNode::~YunetFaceDetectorNode() {
    deinitialized();
}

void YunetFaceDetectorNode::postprocess(const std::vector<cv::Mat> &raw_outputs, const std::vector<std::shared_ptr<tx_objects::FrameMeta>> &frame_meta_with_batch) {
    using namespace cv;
    // 3 heads of output
    ...
    if (loc.rows != priors.size()) {
        inputW = frame_meta->frame_.cols;
        inputH = frame_meta->frame_.rows;
        generatePriors();
    }
    ...
    if (faces.rows > 1) {
        ...
        for (int i = 0; i < nms_faces.rows; i++) {
            ...
            x = std::max(x, 0);
            y = std::max(y, 0);
            w = std::min(w, frame_meta->frame_.cols - x);
            h = std::min(h, frame_meta->frame_.rows - y);
            ...
            auto face_target = std::make_shared<tx_objects::FrameFaceTarget>(x, y, w, h, score, std::vector<std::pair<int, int>>{kp1, kp2, kp3, kp4, kp5});
...
void YunetFaceDetectorNode::preprocess(const std::vector<cv::Mat> &mats_to_infer, cv::Mat &blob_to_infer) {
    cv::dnn::blobFromImages(mats_to_infer, blob_to_infer);
}

// refer to vp_infer_node::infer
void YunetFaceDetectorNode::infer(const cv::Mat &blob_to_infer, std::vector<cv::Mat> &raw_outputs) {
...
void YunetFaceDetectorNode::generatePriors() {
    using namespace cv;
