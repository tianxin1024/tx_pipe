...
SecondaryInferNode::SecondaryInferNode(std::string node_name,
...
SecondaryInferNode::~SecondaryInferNode() {
}

void SecondaryInferNode::prepare(const std::vector<std::shared_ptr<tx_objects::FrameMeta>> &frame_meta_with_batch, std::vector<cv::Mat> &mats_to_infer) {
    ...
    for (auto &i : frame_meta->targets) {
        ...
        if (crop_padding != 0) {
            ...
            box.width = std::min(box.width, frame_meta->frame_.cols - box.x);
            box.height = std::min(box.height, frame_meta->frame_.rows - box.y);
        }

        mats_to_infer.push_back(frame_meta->frame_(box));
    }
}

bool SecondaryInferNode::need_apply(int primary_class_id, int target_width, int target_height) {
