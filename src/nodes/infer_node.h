...
protected:
    ...
    InferNode(std::string node_name,
    ...
    virtual void prepare(const std::vector<std::shared_ptr<tx_objects::FrameMeta>> &frame_meta_with_batch, std::vector<cv::Mat> &mats_to_infer) = 0;
    ...
    virtual void postprocess(const std::vector<cv::Mat> &raw_outputs, const std::vector<std::shared_ptr<tx_objects::FrameMeta>> &frame_meta_with_batch) = 0;
    ...
    virtual void run_infer_combinations(const std::vector<std::shared_ptr<tx_objects::FrameMeta>> &frame_meta_with_batch);
    ...
    virtual std::shared_ptr<tx_objects::Meta> handle_frame_meta(std::shared_ptr<tx_objects::FrameMeta> meta) override final;
    // re-implementation for batch by batch mode, marked as 'final' as we need not override any more in specific derived classes.
    virtual void handle_frame_meta(const std::vector<std::shared_ptr<tx_objects::FrameMeta>> &meta_with_batch) override final;
