...
FaceOsdNodeV2::FaceOsdNodeV2(std::string node_name) :
    Node(node_name) {
    this->initialized();
}

FaceOsdNodeV2::~FaceOsdNodeV2() {
    deinitialized();
}

std::shared_ptr<tx_objects::Meta> FaceOsdNodeV2::handle_frame_meta(std::shared_ptr<tx_objects::FrameMeta> meta) {
    ...
    for (auto &i : meta->face_targets) {
        ...
        if (the_baseline_face.empty()) {
            auto face = meta->frame_(cv::Rect(i->x, i->y, i->width, i->height));
            ...
            for (auto &f : face_features) {
                if (match(i->embeddings, f, 0) >= cosine_similar_thresh || match(i->embeddings, f, 1) <= l2norm_similar_thresh) {
                    exist = true;
                    break;
                }
            }
...
double FaceOsdNodeV2::match(std::vector<float> &feature1, std::vector<float> &feature2, int dis_type) {
