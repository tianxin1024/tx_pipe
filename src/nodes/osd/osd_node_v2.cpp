...
#include "nodes/osd/osd_node_v2.h"
#include "utils/utils.h"

namespace tx_nodes {

OsdNodeV2::OsdNodeV2(std::string node_name, std::string font) :
    Node(node_name) {
    if (!font.empty()) {
...
std::shared_ptr<tx_objects::Meta> OsdNodeV2::handle_frame_meta(std::shared_ptr<tx_objects::FrameMeta> meta) {
    // operations on osd_frame
    if (meta->osd_frame_.empty()) {
        // add a gap at the bottom of osd frame
        meta->osd_frame_ = cv::Mat(meta->frame_.rows + gap_height + padding * 2, meta->frame_.cols, meta->frame_.type(), cv::Scalar(128, 128, 128));

        // initialize by copying frame to osd frame
        auto roi = meta->osd_frame_(cv::Rect(0, 0, meta->frame_.cols, meta->frame_.rows));
        meta->frame_.copyTo(roi);
    }
    auto &canvas = meta->osd_frame_;
    ...
    for (int i = 0; i < meta->targets.size(); i++) {
        ...
        for (int j = 0; j < target->sub_targets.size(); j++) {
            ...
            auto roi = canvas(cv::Rect(base_left, meta->osd_frame_.rows - padding - gap_height, gap_height, gap_height));
            // white background
            ...
            roi = canvas(cv::Rect(base_left + offset_w, meta->osd_frame_.rows - padding - gap_height + offset_h, tmp.cols, tmp.rows));
            tmp.copyTo(roi);

            // line from target to sub target
            cv::line(canvas, cv::Point(target->x + target->width / 2, target->y + target->height),
                     cv::Point(base_left + gap_height / 2, meta->osd_frame_.rows - padding - gap_height), cv::Scalar(255, 0, 0), 3, cv::LINE_AA);

            // label of sub target
            auto sub_label = tx_utils::string_split(sub_target->label, '_');

            // !!!
            // for plate sub target (color_text)
            if (sub_label.size() == 2) {
                assert(ft2 != nullptr);
                ft2->putText(canvas, sub_label[1], cv::Point(base_left + 10, meta->osd_frame_.rows - padding - 5), 36, cv::Scalar(0), cv::FILLED, cv::LINE_AA, true);
            }
