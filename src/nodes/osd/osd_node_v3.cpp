...
#include "nodes/osd/osd_node_v3.h"

namespace tx_nodes {

OsdNodeV3::OsdNodeV3(std::string node_name, std::string font) :
    Node(node_name) {
    if (!font.empty()) {
...
std::shared_ptr<tx_objects::Meta> OsdNodeV3::handle_frame_meta(std::shared_ptr<tx_objects::FrameMeta> meta) {
    // operations on osd_frame
    if (meta->osd_frame_.empty()) {
        meta->osd_frame_ = meta->frame_.clone();
    }
    ...
    for (auto &i : meta->targets) {
        ...
        if (ft2 != nullptr) {
            ...
            tx_utils::put_text_at_center_of_rect(canvas, labels_to_display, cv::Rect(i->x, i->y - size.height, size.width, size.height), true);
        }
        ...
        if (!i->mask.empty()) {
            ...
            cv::Mat coloredRoi = (0.3 * cv::Scalar(255, 255, 0) + 0.7 * meta->osd_frame_(cv::Rect(i->x, i->y, i->width, i->height)));
            ...
            coloredRoi.copyTo(meta->osd_frame_(cv::Rect(i->x, i->y, i->width, i->height)), mask);
        }
    }
    return meta;
}
